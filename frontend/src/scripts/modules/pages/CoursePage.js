...
class CoursePage {
  ...
  init($courseEl) {
    ...
    this.isPublic = $courseEl.data("visibility") == "public" ? true : false;
    this.slideStore = new Store("slides");
  ...
  getLatestSlide() {
    if (this.isPublic) {
      this.passedIds = this.slideStore.getData().reverse();
      this.setActiveSlideOnInit();
    } else {
  ...
  getinitialSlideIndex() {
    ...
    let lastElIndexFromDB = this.SlidesController.slides.find(
      value => value.id == lastSlideIdFromDB
    );
    ...
    if (hash && hash.indexOf("#slide") != -1) {
      ...
      const elementToShow = this.SlidesController.slides.find(
        value => value.index == slideToShow - 1
      );
      const idToShow = elementToShow ? elementToShow.id : {};
      //check if user can go to this step
      if (
        this.passedIds.includes(idToShow) &&
        !(slideToShow > +this.SlidesController.amount || slideToShow <= 0)
      ) {
        this.SlidesController.currentById = idToShow;
        lastSlideIndexFromDB = this.SlidesController.current.data(
          "slide-index"
        );
      } else {
        //user cant go to slide in hash but has some activities so show last activity
        history.replaceState(
          { current: lastSlideIndexFromDB },
          `Slide ${lastSlideIndexFromDB + 1}`,
          `#slide${lastSlideIndexFromDB + 1}`
        );
  ...
  fullscreenChangeHandler(e) {
    ...
    if (!IsFullScreenCurrently()) {
      this.courseEl
        .find(this.selectors.courseControls)
        .removeClass("lms-option-shown");
      this.courseEl.removeClass("lms-fullscreen-init");
      this.courseEl
        .find(this.selectors.courseControls)
        .removeClass("lms-fullscreen-init");
      // this.fullscreenPaintNavButtons(true);

      $(this.selectors.course)
        .find(this.selectors.allTextToResponsive)
        .each(function(i) {
          $(this).css(`fontSize`, ``);
        });
    } else {
  ...
  fullscreenShowToolbarOnHover(event) {
    if (IsFullScreenCurrently()) {
      if (event.pageY >= $(window).height() * 0.8) {
        $(".lms-course-controls.lms-fullscreen-init").addClass(
          "lms-option-shown"
        );
      } else {
        $(".lms-course-controls.lms-fullscreen-init").removeClass(
          "lms-option-shown"
        );
      }
    }
  ...
  setSlideAudio() {
    ...
    if (firstAudioSrc) {
      this.courseEl
        .find(this.selectors.courseControlsAudio)
        .addClass("audio-inited");
      if (isLoop) {
  ...
  listeners() {
    ...
    $(this.selectors.shortcodeBackToCourses).on(
      "click",
      this.shortcodeBackToCourses.bind(this)
    );
    $(this.selectors.shortcodeRestart).on(
      "click",
      this.shortcodeRestart.bind(this)
    );
    ...
    $(this.selectors.slideNavigation).on(
      "click",
      ".next",
      this.nextSlide.bind(this)
    );
    $(this.selectors.sectionNavigation).on(
      "click",
      ".next",
    ...
    $(this.selectors.courseControlsFullscreen).on(
      "click",
      this.toggleFullscreen.bind(this)
    );
    $(this.selectors.fullscreenOptions).on(
      "click",
      this.toggleFullscreenOption.bind(this)
    );
  }
  ...
  fullscreenPaintNavButtons(onCangeFullscreen = false) {
    ...
    $(".lms-course-controls svg .cls-1, .lms-course-controls svg .cls-2").each(
      function(i) {
  ...
  nextSlide(e) {
    ...
    if (this.canGoNext) {
      ...
      if (this.isPublic) {
        const passed = this.slideStore.getData();
        this.slideStore.saveData([...passed, currentId]);
      } else {
        ProgressLogger.commitProgress(
          this.userId,
          this.courseId,
          currentId,
          "finished"
  ...
  showSlide(indexSlide, indexHash, changeUrl = true) {
    ...
    this.slideSectionsCount = this.SlidesController.current.data(
      "section-count"
    );
    this.slideDisplayType = this.SlidesController.current.data(
      "section-display"
    );
    ...
    if (this.SlidesController.current.data("type") === "quiz") {
      if (
        !this.passedIds.includes(this.SlidesController.current.data("slide-id"))
      ) {
      ...
      //init current quiz
      const quizSlide = this.SlidesController.slides.find(
        el => el.id == currentId && el.quiz !== false
      );
      ...
      if (!quizSlide.inited) {
        ...
        // this.canGoNext = true;
        this.courseEl
          .find(this.selectors.slideNavigation)
          .addClass("quiz-unpassed");
  ...
  nextSection(e) {
    ...
    const section = this.SlidesController.current
      .find(this.selectors.gridBlock)
      .eq(nextSectionIndex);
    const firstAudioBlock = slideNode
      .find(this.selectors.audioGridBlock)
      .first();
    // console.log(section);
    ...
    if (slideLayout == "full-width") {
      ...
      $(myElement).animate(
        { scrollTop: section[0].offsetTop + section.outerHeight() },
        400
      );
    }
  ...
  checkCourseNavigation() {
    ...
    if (
      this.slideDisplayType == "all_at_once" ||
      this.slideSectionsCount == 1 ||
      this.SlidesController.current.hasClass("passed")
    ) {
      this.navType = "slide";
      this.enableCourseNav();
      this.disableSectionNav();
  ...
  calculateProgress(
    current = this.SlidesController.current.index(),
    amount = this.SlidesController.amount
  ) {
  ...
  endOfCourse() {
    ...
    $(".lms-button-complete-course").on(
      "click",
      this.confirmCompletionCourse.bind(this)
    );
